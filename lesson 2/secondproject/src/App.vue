<template>
  <div>
    <input :type="test">
    <div v-if="Math.floor(Math.random() * 60) % 6 === 0">Templete</div>
    <div v-else-if="Math.floor(Math.random() * 60) % 6 === 0">Templete</div>
    <div v-else>Templete</div>
    <div v-show="true"></div>
...
const test = ref("text")
if (Math.floor(Math.random() * 60) % 6 === 0) test.value = "number"

const array = ref([2, 3, 4, 54, 5, 555555555555555, 123, 12, 3, 123])

setTimeout(() => {
  array.value.push(675)
}, 3000);
setTimeout(() => {
  array.value.pop()
}, 7000);
